module CryTUI
  ...
  #
  # A widget picks each span's colour for the ordinary background — an item
  # count in a muted grey, a shortcut in amber. When a row is repainted in
  # different colours, some of those choices stop working: on a dim selection
  # bar a muted grey can come within 1.05:1 of the bar itself, which is
  # invisible. Rather than flatten every span to one colour, which would also
  # erase the ones that are perfectly readable and carrying meaning, this
  # replaces only the ones that fail.
