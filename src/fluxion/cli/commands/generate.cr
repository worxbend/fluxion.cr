module Fluxion::CLI
  ...
  class GenerateCommand < Command
    ...
    private def render(distribution : Distribution) : String
      ...
      String.build do |io|
        ...
        io << "profile: " << @profile_name << '\n'
        io << "os:\n"
        io << "  type: " << distribution.config_name << '\n'
        Host.facts.version.try { |version| io << "  release: \"" << version << "\"\n" }
        io << '\n'
        io << "jobs:\n"
        io << "  - name: base\n"
        io << "    steps:\n"
  ...
  abstract class ImportSubcommand < Command
    ...
    #
    # The Java version emitted just the job, which meant the output could not
    # be validated or previewed without hand-editing a header onto it first.
    # Emitting the header costs two lines and loses nothing: the job below it
    # still lifts straight into an existing profile.
    protected def header(io : IO, name : String) : Nil
      facts = Host.facts
      distribution = facts.distribution
      io << "profile: " << name << '\n'
      io << "os:\n"
      io << "  type: " << (distribution.try(&.config_name) || "fedora") << '\n'
      facts.version.try { |version| io << "  release: \"" << version << "\"\n" }
      io << '\n'
    end
