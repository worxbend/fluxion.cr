module Fluxion::Config
  ...
  module Manifest
    ...
    def parse(context : Context, root : Node, path : String) : Profile
      ...
      parsing = PhaseParsing.new(context, target)
      phases = parsing.phases(spec["phases"])
    ...
    # One parse of one profile's phases and steps.
    #
    # Every method below needs the same four collaborators: the context, the
    # target OS, the name registries and the list of skipped entries. They are
    # fields of an object that lives for the length of one parse rather than a
    # convoy threaded through every signature.
    private class PhaseParsing
      ...
      def initialize(@context : Context, @target : TargetOs)
        @names = Names.new
        @skipped = [] of SkippedPlanEntry
      end

      def phases(node : Node) : Array(Phase)
        phases = [] of Phase
      ...
      private def parse_phase(entry : Node) : Phase
        ...
        restart_policy = parse_restart_policy(entry["restartPolicy"])
      ...
      private def parse_restart_policy(node : Node) : RestartPolicy
        ...
        when "requires-new-shell"
          RestartPolicy::RequiresNewShell.new(@context.optional_enum(node["shell"], "shell", ShellKind::Zsh))
        else
      ...
      private def parse_step(entry : Node, phase : String, phase_unmet : String?) : Step?
        name = @context.optional_string(entry["name"])
        if name.nil?
          @context.error(entry["name"].path, "must not be blank")
        ...
        build_step(entry, kind, label, condition)
      end

      private def step_kind(entry : Node) : PlanKinds::Kind?
        node = entry["kind"]
        raw = node.string?
        if raw.nil? || raw.strip.empty?
      ...
      private def build_step(entry : Node, kind : PlanKinds::Kind,
                             name : String, condition : Condition?) : Step?
        spec = entry["spec"]
        description = @context.optional_string(entry["description"])
        ...
        step = build_kind_step(spec, kind, name, description, probe)
        return unless step

        validate_package_manager(step, entry, condition)
      ...
      private def build_kind_step(spec : Node, kind : PlanKinds::Kind, name : String, description : String?, probe : String?) : Step?
        case kind.category
        ...
        in PlanKinds::Category::Control
          ...
          build_by_step_type(spec, kind, name, description, probe)
      ...
      private def build_by_step_type(spec : Node, kind : PlanKinds::Kind, name : String, description : String?, probe : String?) : Step?
      ...
      private def validate_package_manager(step : Step, entry : Node, condition : Condition?) : Nil
    ...
    private record SourceSection, manager : PackageManager, kind : String

    # Each section maps onto the package manager whose entries it configures
    # and the step kind its entries are built as. `rpm` is an alias for `dnf`,
    # and `pacman` is accepted by the schema but produces no setup operations —
    # a pacman mirror is not a finite artifact Fluxion can verify.
    SOURCE_SECTIONS = {
      "apt"     => SourceSection.new(PackageManager::Apt, "apt-repository"),
      "dnf"     => SourceSection.new(PackageManager::Dnf, "rpm-repository"),
      "rpm"     => SourceSection.new(PackageManager::Dnf, "rpm-repository"),
      "zypper"  => SourceSection.new(PackageManager::Zypper, "zypper-repository"),
      "flatpak" => SourceSection.new(PackageManager::Flatpak, "flatpak-remote"),
    }
    ...
    private def parse_sources(context : Context, node : Node, steps : Array(Step)) : {Array(SourceSetup), Array(SkippedPlanEntry)}
      ...
      SOURCE_SECTIONS.each do |section, source|
        ...
        section_node.each_item do |entry, _|
          ...
          unless required.includes?(source.manager)
            skipped << SkippedPlanEntry.new(name, "#{section}-source",
              "source section #{section} is not relevant to selected host package managers")
            next
          end

          step = StepParser.build_kind(context, entry["spec"], source.kind, name, nil, nil)
          setups << SourceSetup.new(step, source.manager) if step
        end
      end
